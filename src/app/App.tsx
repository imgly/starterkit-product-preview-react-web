...
export default function App({ config }: AppProps) {
  ...
  const handleEditorInit = useCallback(
    async (cesdk: CreativeEditorSDK) => {
      ...
      // Load initial design scene
      const initialSceneUrl = getDesignSceneUrl(currentProductKey);
      await cesdk.engine.scene.loadFromURL(initialSceneUrl);

      // Render initial mockup (engine initializes lazily on first render)
      await renderMockupForProduct(currentProductKey);
    },
    [currentProductKey, renderMockupForProduct]
  );
